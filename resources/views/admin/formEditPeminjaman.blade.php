    ...
    <div class="flex">
        ...
        <div class="flex-1 p-10 content content-expanded" id="mainContent">
            ...
            <div class="bg-white p-8 rounded-lg shadow-lg">
                <form method="POST" action="{{route('admin.editPeminjamanAction', $peminjaman->id)}}">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-gray-700">Nama Peminjam</label>
                            <input type="text" class="w-full mt-2 p-2 border rounded" value="{{$peminjaman->user->username}}" readonly>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="buku" id="buku_id" class="w-full mt-2 p-2 border rounded">
                                @foreach ($buku as $item)
                                    <option value="{{$item->id}}" @if($item->id == $peminjaman->buku->id) selected @endif>{{$item->judul}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
