    ...
    <div class="flex">
        ...
        <div class="flex-1 p-10 content content-expanded" id="mainContent">
            ...
            <div class="bg-white p-8 rounded-lg shadow-lg">
                <form method="POST" action="{{route('admin.editUserAction', $user->id)}}">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-gray-700">Nama Lengkap</label>
                            <input name="username" class="w-full mt-2 p-2 border rounded" type="text" value="{{$user->username}}"/>
                        </div>
                        <div>
                            <label class="block text-gray-700">Password</label>
                            <input type= "password" name="penulis" class="w-full mt-2 p-2 border rounded" type="text" value="{{$user->password}}"/>
                        </div>
                        <div>
                            <label class="block text-gray-700">Email</label>
                            <input name="email" class="w-full mt-2 p-2 border rounded" type="text" value="{{$user->email}}"/>
                        </div>
                        <div>
                            <label class="block text-gray-700">Role</label>
                            <select class="w-full mt-2 p-2 border rounded" name="role">
